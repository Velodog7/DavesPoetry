...
async function route(req, res, url) {
  ...
  if (!resource || resource === 'health') {
    return send(res, 200, {
      ...
      poems: (await store.read()).poems.length,
  ...
  if (!auth.isConfigured(data)) {
    ...
    /* The same goes for the example poems. They are the deploy's placeholders,
       not anyone's writing — so the deploy decides which of them exist, right
       up until someone owns the site. Ship none and none are served, without
       waiting for an account to be created. Anything the poet has actually
       touched lost its sample flag when he saved it, and is left alone. */
