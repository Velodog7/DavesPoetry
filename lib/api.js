  ...
  if (resource === 'auth') {
    ...
    if (action === 'setup' && method === 'POST') {
      ...
      if (!already) {
        ...
        /* Until the site has an owner its identity belongs to the deploy, so
           take the name from the seed. Nothing anyone chose is being
           overwritten — nobody had the standing to choose it yet. Once the
           account exists, only the author changes this, through /api/site. */
        const seed = readSeed();
