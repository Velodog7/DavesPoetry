...
function applyDeployDefaults(data) {
  const seed = readSeed();
  const site = (seed && seed.site) || {};

  /* Once he has an account the name is his and the deploy keeps its hands off.
     One exception: a name the deploy itself put there and he never changed.
     Renaming that is correcting our own text, not overruling his — and the
     moment he types something of his own it stops matching this list and is
     his for good. */
  if (auth.isConfigured(data)) {
    const ours = Array.isArray(site.previousNames) ? site.previousNames : [];
    if (site.name && ours.indexOf(data.site.name) >= 0) data.site.name = site.name;
    return data;
  }

  if (site.name) {
    data.site.name = site.name;
    if (site.byline) data.site.byline = site.byline;
  }
    ...
    if (method === 'GET') {
      ...
      if (search) {
        list = list.filter(p =>
          ...
          /* Search the words, not the markers — *loud* must be found by "loud". */
          engine.stripMarks(p.body).toLowerCase().includes(search) ||
