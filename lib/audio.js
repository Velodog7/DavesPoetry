 ...
 *       "duration": 56.61,
 *       "credit": "",                   // shown beside "Listen"; omit when unattributed
 *       "leadIn": 1.98,                 // documentation; see below
 *       "fingerprint": "2d526b611c5f",  // of the body the timings were cut to
 *       "words": [[start, end], ...]    // seconds, one pair per word, in order
 *   } }
 *
 * A reader usually announces the title before starting the poem, so word 0
 * does not begin at zero. The aligner must be given the title as part of its
 * text and have those words dropped afterwards; align the poem alone and the
 * whole poem is stretched back over the title, which puts every word early —
 * badly at the opening, converging by the end. `leadIn` records where the poem
 * itself starts and exists to make that visible in the data; nothing reads it.
 *
