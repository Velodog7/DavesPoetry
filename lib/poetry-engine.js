...
function excerpt(body,n){
  var lines=stripMarks(body).split(/\r?\n/).filter(function(l){ return l.trim()!==''; });
...
/* ================= AUTHOR HIGHLIGHTS ================= */
/* ================= INLINE FORMATTING =================
   The poet writes in a plain textarea, so emphasis has to live in the text
   itself: *italic*, **bold**, ~~struck through~~, and \* for a literal marker.

   The rule that keeps everything else working: nothing that counts, indexes or
   analyses a poem ever sees the markers. Highlights are stored as word
   positions, and a word's position must not change because someone italicised
   it. So parseMarks() returns the plain text plus a per-character style map,
   and every other function in this file works from the plain text. */
var FMT_EM=1, FMT_STRONG=2, FMT_STRIKE=4;

/* Two passes. The first decides which markers are genuinely paired; the second
   builds the text. An unpaired marker stays as literal text — a poem is
   allowed to contain a lone asterisk without turning the rest of it italic. */
function parseMarks(body){
  ...
  while(i<src.length){
    var c=src[i];
    if(c==='\\' && i+1<src.length){ i+=2; continue; }      /* escaped: skip both */
    var two=src.substr(i,2), tok=null;
    if(two==='**') tok={s:'**',bit:FMT_STRONG,len:2};
    else if(two==='~~') tok={s:'~~',bit:FMT_STRIKE,len:2};
    else if(c==='*') tok={s:'*',bit:FMT_EM,len:1};
    if(tok){
      var openAt=-1;
      for(var k=stack.length-1;k>=0;k--){ if(stack[k].bit===tok.bit){ openAt=k; break; } }
      if(openAt>=0){
        var open=stack.splice(openAt,1)[0];
        marks.push({at:open.at,len:open.len,bit:open.bit,close:false});
        marks.push({at:i,len:tok.len,bit:tok.bit,close:true});
      } else {
        stack.push({at:i,len:tok.len,bit:tok.bit});
      }
      i+=tok.len; continue;
    }
    i++;
  ...
  var paired={};
  marks.forEach(function(m){ paired[m.at]=m; });

  var plain='', fmt=[], active=0, map={};
  marks.forEach(function(m){ map[m.at]=m; });
  i=0;
  while(i<src.length){
    if(src[i]==='\\' && i+1<src.length){
      plain+=src[i+1]; fmt.push(active); i+=2; continue;
    }
    var m=map[i];
    if(m){ active = m.close ? (active & ~m.bit) : (active | m.bit); i+=m.len; continue; }
    plain+=src[i]; fmt.push(active); i++;
  }
  return { plain:plain, fmt:fmt };
}
function stripMarks(body){ return parseMarks(body).plain; }
/* True when the text carries any formatting the poet actually applied. */
function hasMarks(body){ return stripMarks(body)!==String(body==null?'':body); }
...
/* Split a poem into word tokens (indexed) and the gaps between them. Gaps keep
   ...
   Always operates on the plain text — see parseMarks above. */
function tokenizePlain(plain){
  var re=new RegExp(WORD_RE.source,'g'), toks=[], last=0, m, wi=0;
  while((m=re.exec(plain))!==null){
    if(m.index>last) toks.push({t:'gap',s:plain.slice(last,m.index),a:last,b:m.index});
    toks.push({t:'word',s:m[0],i:wi++,a:m.index,b:m.index+m[0].length});
    last=m.index+m[0].length;
  }
  if(last<plain.length) toks.push({t:'gap',s:plain.slice(last),a:last,b:plain.length});
  return toks;
}
function tokenizePoem(body){ return tokenizePlain(stripMarks(body)); }
...
function fmtSlice(plain,fmt,a,b){
  var out='', i=a;
  while(i<b){
    var f=fmt[i]||0, j=i;
    while(j<b && (fmt[j]||0)===f) j++;
    var piece=esc(plain.slice(i,j));
    if(f&FMT_STRIKE) piece='<s>'+piece+'</s>';
    if(f&FMT_EM)     piece='<em>'+piece+'</em>';
    if(f&FMT_STRONG) piece='<strong>'+piece+'</strong>';
    out+=piece; i=j;
  }
  return out;
}
function renderVerseHtml(body,indices,mode,sync){
  var pm=parseMarks(body);
  var set={}; (indices||[]).forEach(function(i){ set[i]=1; });
  var n=0, line=0;
  return tokenizePlain(pm.plain).map(function(tk){
    var inner=fmtSlice(pm.plain,pm.fmt,tk.a,tk.b);
    if(tk.t==='gap'){
      line+=(tk.s.match(/\n/g)||[]).length;
      return inner;
    }
    ...
    if(mode==='pick'){
      return '<span class="wtok'+(on?' on':'')+'" data-wi="'+tk.i+'" role="button" tabindex="0"'+
        ' aria-pressed="'+on+'" aria-label="'+esc(tk.s)+'">'+inner+'</span>';
    }
    var out=on ? '<mark class="hl" style="--d:'+(n++*45)+'ms">'+inner+'</mark>' : inner;
    if(!sync) return out;
    return '<span class="wsync" data-wi="'+tk.i+'" data-li="'+line+'">'+out+'</span>';
  }).join('');
}
...
function tokenize(text){
  /* The curly apostrophe counts. Without it "yesterday’s" was two words here
     and one word everywhere else, so the analysis reported a higher count than
     the poem actually has. */
  var m=(stripMarks(text).toLowerCase().match(/[a-z][a-z'’]*/g))||[];
  return m.map(function(w){ return w.replace(/^['’]+|['’]+$/g,''); }).filter(Boolean);
}
function getLines(body){ return stripMarks(body).split(/\r?\n/).filter(function(l){ return l.trim()!==''; }); }
function getStanzas(body){
  return stripMarks(body).split(/\n\s*\n/).map(function(s){ return s.split(/\r?\n/).filter(function(l){ return l.trim()!==''; }); })
...
module.exports = {
  /* inline formatting */
  parseMarks, stripMarks, hasMarks, renderVerseHtml,
  /* text → tokens */
